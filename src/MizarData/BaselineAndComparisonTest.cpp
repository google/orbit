...
template <size_t N>
static absl::flat_hash_map<Address, std::string> MakeAddressToNameMap(
    const std::array<uint64_t, N>& raw_addresses, const std::array<std::string, N>& names) {
  std::array<Address, N> addresses;
  absl::c_transform(raw_addresses, std::begin(addresses),
                    [](uint64_t raw) { return Address(raw); });
  return MakeMap(addresses, names);
}

const absl::flat_hash_map<Address, std::string> kBaselineAddressToName =
    MakeAddressToNameMap(kBaselineFunctionAddresses, kBaselineFunctionNames);
const absl::flat_hash_map<Address, std::string> kComparisonAddressToName =
    MakeAddressToNameMap(kComparisonFunctionAddresses, kComparisonFunctionNames);

static void ExpectCorrectNames(const absl::flat_hash_map<Address, SFID>& address_to_sfid,
                               const absl::flat_hash_map<SFID, std::string>& sfid_to_name,
                               const absl::flat_hash_map<Address, std::string>& address_to_name) {
