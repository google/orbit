...
int main(int argc, char* argv[]) {
  ...
  orbit_grpc_protos::DynamicInstrumentationMethod instrumentation_method =
      absl::GetFlag(FLAGS_user_space_instrumentation)
          ? orbit_grpc_protos::DynamicInstrumentationMethod::kOrbit
          : orbit_grpc_protos::DynamicInstrumentationMethod::kKernelUprobes;
  LOG("user_space_instrumentation=%d",
      instrumentation_method == orbit_grpc_protos::DynamicInstrumentationMethod::kOrbit);
  if (instrument_function) {
    LOG("file_path=%s", file_path);
    LOG("file_offset=%#x", file_offset);
    if (instrumentation_method == orbit_grpc_protos::DynamicInstrumentationMethod::kOrbit) {
