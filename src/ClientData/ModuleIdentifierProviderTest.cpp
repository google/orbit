...
TEST(ModuleIdentifierProvider, CanCreateModuleIdentifiers) {
  ...
  ModuleIdentifier module_id1 =
      module_identifier_provider.CreateModuleIdentifier(kModulePath, kBuildId);
  ModuleIdentifier module_id2 =
      module_identifier_provider.CreateModuleIdentifier(kModulePath, kBuildId);

  EXPECT_EQ(module_id1, module_id2);

  constexpr std::string_view kDifferentModulePath = "/path/to/different/module";
  ModuleIdentifier module_id_different_path =
      module_identifier_provider.CreateModuleIdentifier(kDifferentModulePath, kBuildId);

  EXPECT_NE(module_id1, module_id_different_path);

  constexpr std::string_view kDifferentBuildId = "build_id2";
  ModuleIdentifier module_id_different_build_id =
      module_identifier_provider.CreateModuleIdentifier(kModulePath, kDifferentBuildId);
...
TEST(ModuleIdentifierProvider, ReturnsNulloptsForUnknownModule) {
  ...
  std::optional<ModuleIdentifier> module_identifier_opt =
      module_identifier_provider.GetModuleIdentifier(kUnknownModulePath, kUnknownBuildId);
  ...
  ModuleIdentifier unknown_module_identifier =
      another_module_identifier_provider.CreateModuleIdentifier(kUnknownModulePath,
                                                                kUnknownBuildId);
...
TEST(ModuleIdentifierProvider, CanCreateMultipleModuleIdentifiersAndReturnInformationForThem) {
  ...
  ModuleIdentifier module_identifier1 =
      module_identifier_provider.CreateModuleIdentifier(kModulePath1, kBuildId1);

  std::optional<ModuleIdentifier> module_identifier1_opt =
      module_identifier_provider.GetModuleIdentifier(kModulePath1, kBuildId1);
  ...
  static const std::pair<std::string, std::string> kExpectedModulePathAndBuildId1{
      std::string{kModulePath1}, std::string{kBuildId1}};
  ...
  ModuleIdentifier module_identifier2 =
      module_identifier_provider.CreateModuleIdentifier(kModulePath2, kBuildId2);

  EXPECT_NE(module_identifier1, module_identifier2);
  EXPECT_NE(module_identifier_provider.GetModuleIdentifier(kModulePath1, kBuildId1),
            module_identifier_provider.GetModuleIdentifier(kModulePath2, kBuildId2));
  ...
  static const std::pair<std::string, std::string> kExpectedModulePathAndBuildId2{
      std::string{kModulePath2}, std::string{kBuildId2}};
