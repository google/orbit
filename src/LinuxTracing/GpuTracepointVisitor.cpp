...
void GpuTracepointVisitor::Visit(AmdgpuCsIoctlPerfEvent* event) {
  AmdgpuCsIoctlEvent internal_event{event->data().pid,   event->data().tid,
                                    event->timestamp(),  event->data().context,
                                    event->data().seqno, event->data().timeline_string};
  Key key =
      std::make_tuple(event->data().context, event->data().seqno, event->data().timeline_string);
...
void GpuTracepointVisitor::Visit(AmdgpuSchedRunJobPerfEvent* event) {
  AmdgpuSchedRunJobEvent internal_event{event->timestamp(), event->data().context,
                                        event->data().seqno, event->data().timeline_string};
  Key key =
      std::make_tuple(event->data().context, event->data().seqno, event->data().timeline_string);
...
void GpuTracepointVisitor::Visit(DmaFenceSignaledPerfEvent* event) {
  DmaFenceSignaledEvent internal_event{event->timestamp(), event->data().context,
                                       event->data().seqno, event->data().timeline_string};
  Key key =
      std::make_tuple(event->data().context, event->data().seqno, event->data().timeline_string);
