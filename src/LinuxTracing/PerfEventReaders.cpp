...
MmapPerfEvent ConsumeMmapPerfEvent(PerfEventRingBuffer* ring_buffer,
  ...
  return MmapPerfEvent{ring_buffer->GetFileDescriptor(),
                       timestamp,
                       mmap_event.address,
                       mmap_event.length,
                       mmap_event.page_offset,
                       std::move(filename),
                       pid};
}
...
StackSamplePerfEvent ConsumeStackSamplePerfEvent(PerfEventRingBuffer* ring_buffer,
  ...
  StackSamplePerfEvent event{ring_buffer->GetFileDescriptor(), sample_id.time,
                             static_cast<pid_t>(sample_id.pid), static_cast<pid_t>(sample_id.tid),
                             dyn_size};
...
CallchainSamplePerfEvent ConsumeCallchainSamplePerfEvent(PerfEventRingBuffer* ring_buffer,
  ...
  CallchainSamplePerfEvent event{ring_buffer->GetFileDescriptor(),
                                 sample_id.time,
                                 static_cast<pid_t>(sample_id.pid),
                                 static_cast<pid_t>(sample_id.tid),
                                 nr,
                                 dyn_size};
...
GenericTracepointPerfEvent ConsumeGenericTracepointPerfEvent(PerfEventRingBuffer* ring_buffer,
  ...
  GenericTracepointPerfEvent event{
      ring_buffer->GetFileDescriptor(), ring_buffer_record.sample_id.time,
      static_cast<pid_t>(ring_buffer_record.sample_id.pid),
      static_cast<pid_t>(ring_buffer_record.sample_id.tid), ring_buffer_record.sample_id.cpu};
...
EventType ConsumeGpuEvent(PerfEventRingBuffer* ring_buffer, const perf_event_header& header) {
  ...
  EventType event{kNotOrderedInAnyFileDescriptor,
                  ring_buffer_record.sample_id.time,
                  static_cast<pid_t>(ring_buffer_record.sample_id.pid),
                  static_cast<pid_t>(ring_buffer_record.sample_id.tid),
                  typed_tracepoint_data.context,
                  typed_tracepoint_data.seqno,
                  std::string(&data_loc_data[0])};
