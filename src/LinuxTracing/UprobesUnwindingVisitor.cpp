...
template <typename StackEvent>
bool UprobesUnwindingVisitor::UnwindStack(Callstack* callstack, pid_t tid, pid_t pid,
                                          const StackEvent& event) {
  ORBIT_CHECK(listener_ != nullptr);
  ORBIT_CHECK(current_maps_ != nullptr);

  return_address_manager_->PatchSample(tid, event.GetRegisters().sp, event.GetMutableStackData(),
                                       event.GetStackSize());

  StackSliceView event_stack_slice{event.GetRegisters().sp, event.GetStackSize(),
                                   event.GetStackData()};
  std::vector<StackSliceView> stack_slices{event_stack_slice};
  const auto& stream_id_to_user_stack = thread_id_stream_id_to_stack_slices_.find(tid);
  ...
  LibunwindstackResult libunwindstack_result =
      unwinder_->Unwind(pid, current_maps_->Get(), event.GetRegistersAsArray(), stack_slices);
...
void UprobesUnwindingVisitor::Visit(uint64_t event_timestamp,
  ...
  bool const success =
      UnwindStack(sample.mutable_callstack(), event_data.tid, event_data.pid, event_data);
...
void UprobesUnwindingVisitor::Visit(uint64_t event_timestamp,
  ...
  bool const success =
      UnwindStack(tracepoint_callstack.mutable_callstack(), event_data.was_unblocked_by_tid,
                  event_data.was_unblocked_by_pid, event_data);
...
void UprobesUnwindingVisitor::Visit(uint64_t event_timestamp,
  ...
  TracepointCallstack tracepoint_callstack;
  tracepoint_callstack.set_tid(event_data.prev_tid);
  tracepoint_callstack.set_timestamp_ns(event_timestamp);
  bool const success = UnwindStack(tracepoint_callstack.mutable_callstack(), event_data.prev_tid,
                                   event_data.prev_pid_or_minus_one, event_data);
