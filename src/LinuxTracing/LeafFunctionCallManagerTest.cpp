...
class LeafFunctionCallManagerTest : public ::testing::Test {
 protected:
  ...
  static constexpr uint32_t kMaxStackDumpSize = 128;

  LeafFunctionCallManager leaf_function_call_manager_{kMaxStackDumpSize};
...
TEST_F(LeafFunctionCallManagerTest, PatchCallerOfLeafFunctionReturnsErrorOnSmallStackSamples) {
  ...
  event.regs.bp = 2 * kMaxStackDumpSize;
  event.regs.sp = 0;
...
TEST_F(LeafFunctionCallManagerTest, PatchCallerOfLeafFunctionReturnsErrorOnUnwindingErrors) {
  ...
  event.ring_buffer_record.sample_id = sample_id;
  event.ips = callchain;
  event.regs.bp = kMaxStackDumpSize;
  event.regs.sp = 10;
  ...
  EXPECT_CALL(unwinder_, Unwind(kPid, nullptr, _, _, kMaxStackDumpSize - 10, _, _))
      .Times(1)
