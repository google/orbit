...
PerfEvent MakeFakeAmdgpuCsIoctlPerfEvent(pid_t pid, pid_t tid, uint64_t timestamp_ns,
                                         uint32_t context, uint32_t seqno,
                                         const std::string& timeline) {
  return PerfEvent{
      .timestamp = timestamp_ns,
      .data =
          AmdgpuCsIoctlPerfEvent{
              .pid = pid,
              .tid = tid,
              .context = context,
              .seqno = seqno,
...
PerfEvent MakeFakeAmdgpuSchedRunJobPerfEvent(uint64_t timestamp_ns, uint32_t context,
                                             uint32_t seqno, const std::string& timeline) {
  return PerfEvent{
      .timestamp = timestamp_ns,
      .data =
          AmdgpuSchedRunJobPerfEvent{
              .context = context,
              .seqno = seqno,
              .timeline_string = timeline,
          },
...
PerfEvent MakeFakeDmaFenceSignaledPerfEvent(uint64_t timestamp_ns, uint32_t context, uint32_t seqno,
                                            const std::string& timeline) {
  return PerfEvent{
      .timestamp = timestamp_ns,
      .data =
