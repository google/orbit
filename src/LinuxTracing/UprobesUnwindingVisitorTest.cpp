...
StackSamplePerfEvent BuildFakeStackSamplePerfEvent() {
  ...
  return StackSamplePerfEvent(kNotOrderedInAnyFileDescriptor, kTimestamp, kPid, kTid, kStackSize);
}
...
CallchainSamplePerfEvent BuildFakeCallchainSamplePerfEvent(const std::vector<uint64_t>& callchain) {
  ...
  CallchainSamplePerfEvent event{
      kNotOrderedInAnyFileDescriptor, kTimestamp, kPid, kTid, callchain.size(), kStackSize};
...
TEST_F(UprobesUnwindingVisitorTest,
  ...
  {
    UprobesPerfEvent uprobe1{kNotOrderedInAnyFileDescriptor,
                             /*timestamp=*/100,
                             kPid,
                             kTid,
                             kCpu,
...

  {
    UprobesWithArgumentsPerfEvent uprobe2{
        ...
        /*function_id=*/2,
        /*return_address=*/0x01,
        perf_event_sample_regs_user_sp_ip_arguments{
            .cx = 4, .dx = 3, .si = 2, .di = 1, .sp = 0x30, .ip = 0x02, .r8 = 5, .r9 = 6}};

    EXPECT_CALL(return_address_manager_, ProcessUprobes(kTid, 0x30, 0x01)).Times(1);
    uprobe2.Accept(&visitor_);
    Mock::VerifyAndClearExpectations(&return_address_manager_);
  }

  {
    UprobesPerfEvent uprobe3{kNotOrderedInAnyFileDescriptor,
                             /*timestamp=*/300,
                             kPid,
                             kTid,
                             kCpu,
...

  {
    UprobesWithArgumentsPerfEvent uprobe4{
        ...
        /*return_address=*/0x03,
        perf_event_sample_regs_user_sp_ip_arguments{
            .cx = 4, .dx = 3, .si = 2, .di = 1, .sp = 0x10, .ip = 0x04, .r8 = 5, .r9 = 6}};

    EXPECT_CALL(return_address_manager_, ProcessUprobes(kTid, 0x10, 0x03)).Times(1);
    uprobe4.Accept(&visitor_);
    Mock::VerifyAndClearExpectations(&return_address_manager_);
  }

  {
    UretprobesWithReturnValuePerfEvent uretprobe4{kNotOrderedInAnyFileDescriptor, /*timestamp=*/500,
                                                  kPid, kTid, /*rax=*/456};
  ...
  {
    UretprobesWithReturnValuePerfEvent uretprobe3{kNotOrderedInAnyFileDescriptor, /*timestamp=*/600,
                                                  kPid, kTid, /*rax=*/123};

    EXPECT_CALL(return_address_manager_, ProcessUretprobes(kTid)).Times(1);
    orbit_grpc_protos::FunctionCall actual_function_call;
    EXPECT_CALL(listener_, OnFunctionCall).Times(1).WillOnce(SaveArg<0>(&actual_function_call));
    uretprobe3.Accept(&visitor_);
    Mock::VerifyAndClearExpectations(&return_address_manager_);
    Mock::VerifyAndClearExpectations(&listener_);
    EXPECT_EQ(actual_function_call.pid(), kPid);
    EXPECT_EQ(actual_function_call.tid(), kTid);
  ...
  {
    UretprobesPerfEvent uretprobe2{kNotOrderedInAnyFileDescriptor, /*timestamp=*/700, kPid, kTid};

    EXPECT_CALL(return_address_manager_, ProcessUretprobes(kTid)).Times(1);
    orbit_grpc_protos::FunctionCall actual_function_call;
    EXPECT_CALL(listener_, OnFunctionCall).Times(1).WillOnce(SaveArg<0>(&actual_function_call));
    uretprobe2.Accept(&visitor_);
    Mock::VerifyAndClearExpectations(&return_address_manager_);
    Mock::VerifyAndClearExpectations(&listener_);
    EXPECT_EQ(actual_function_call.pid(), kPid);
    EXPECT_EQ(actual_function_call.tid(), kTid);
  ...
  {
    UretprobesPerfEvent uretprobe1{kNotOrderedInAnyFileDescriptor, /*timestamp=*/800, kPid, kTid};

    EXPECT_CALL(return_address_manager_, ProcessUretprobes(kTid)).Times(1);
    orbit_grpc_protos::FunctionCall actual_function_call;
    EXPECT_CALL(listener_, OnFunctionCall).Times(1).WillOnce(SaveArg<0>(&actual_function_call));
    uretprobe1.Accept(&visitor_);
    Mock::VerifyAndClearExpectations(&return_address_manager_);
    Mock::VerifyAndClearExpectations(&listener_);
    EXPECT_EQ(actual_function_call.pid(), kPid);
    EXPECT_EQ(actual_function_call.tid(), kTid);
...
TEST_F(UprobesUnwindingVisitorTest,
  ...
  auto fake_patch_caller_of_leaf_function = [](CallchainSamplePerfEvent* event,
    ...
    EXPECT_THAT(event->data().CopyOfIpsAsVector(),
    ...
    event->data().SetIps(patched_callchain);
