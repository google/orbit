...
TEST(GetAllPids, LinuxTracingTestsAndSystemd) {
...
TEST(GetTidsOfProcess, LinuxTracingTestsMainAndAnother) {
...
TEST(GetAllTids, LinuxTracingTestsMainAndAnotherAndSystemd) {
  pid_t main_tid = syscall(SYS_gettid);
  pid_t thread_tid = -1;
  std::vector<pid_t> returned_tids{};
