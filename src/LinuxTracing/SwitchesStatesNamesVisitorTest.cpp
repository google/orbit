...
ForkPerfEvent MakeFakeForkPerfEvent(pid_t pid, pid_t tid) {
  return ForkPerfEvent{kNotOrderedInAnyFileDescriptor, 0, pid, tid};
}

ExitPerfEvent MakeFakeExitPerfEvent(pid_t pid, pid_t tid) {
  return ExitPerfEvent{kNotOrderedInAnyFileDescriptor, 0, pid, tid};
}

TaskNewtaskPerfEvent MakeFakeTaskNewtaskPerfEvent(pid_t new_tid, const char* comm,
                                                  uint64_t timestamp_ns) {
  CHECK(strlen(comm) < 16);
  return TaskNewtaskPerfEvent{kNotOrderedInAnyFileDescriptor, timestamp_ns, comm, new_tid};
}

TaskRenamePerfEvent MakeFakeTaskRenamePerfEvent(pid_t renamed_tid, const char* new_comm,
                                                uint64_t timestamp_ns) {
  CHECK(strlen(new_comm) < 16);
  return TaskRenamePerfEvent{kNotOrderedInAnyFileDescriptor, timestamp_ns, new_comm, renamed_tid};
}
...
SchedSwitchPerfEvent MakeFakeSchedSwitchPerfEvent(uint32_t cpu, pid_t prev_pid_or_minus_one,
  ...
  return SchedSwitchPerfEvent{kNotOrderedInAnyFileDescriptor,
                              timestamp_ns,
                              cpu,
                              prev_pid_or_minus_one,
                              prev_tid,
                              prev_state_mask,
                              next_tid};
}
...
SchedWakeupPerfEvent MakeFakeSchedWakeupPerfEvent(pid_t woken_tid, uint64_t timestamp_ns) {
  return SchedWakeupPerfEvent{kNotOrderedInAnyFileDescriptor, timestamp_ns, woken_tid};
}
