...
void SwitchesStatesNamesVisitor::Visit(ForkPerfEvent* event) {
  pid_t pid = event->data().pid;
  pid_t tid = event->data().tid;
  bool new_insertion = tid_to_pid_association_.insert_or_assign(tid, pid).second;
  if (!new_insertion) {
...
void SwitchesStatesNamesVisitor::Visit(TaskNewtaskPerfEvent* event) {
  std::optional<pid_t> new_pid = GetPidOfTid(event->data().new_tid);
  ThreadName thread_name;
  thread_name.set_pid(new_pid.value_or(-1));
  thread_name.set_tid(event->data().new_tid);
  thread_name.set_name(event->data().comm);
  thread_name.set_timestamp_ns(event->timestamp());
  listener_->OnThreadName(std::move(thread_name));

  if (!TidMatchesPidFilter(event->data().new_tid)) {
    return;
  }
  state_manager_.OnNewTask(event->timestamp(), event->data().new_tid);
}
...
void SwitchesStatesNamesVisitor::Visit(SchedSwitchPerfEvent* event) {
  ...
  if (produce_scheduling_slices_ && event->data().prev_tid != 0) {
    ...
    pid_t prev_pid = event->data().prev_pid_or_minus_one;
    if (prev_pid == -1) {
      if (std::optional<pid_t> fallback_prev_pid = GetPidOfTid(event->data().prev_tid);
    ...
    std::optional<SchedulingSlice> scheduling_slice = switch_manager_.ProcessContextSwitchOut(
        prev_pid, event->data().prev_tid, event->data().cpu, event->timestamp());
  ...
  if (produce_scheduling_slices_ && event->data().next_tid != 0) {
    std::optional<pid_t> next_pid = GetPidOfTid(event->data().next_tid);
    switch_manager_.ProcessContextSwitchIn(next_pid, event->data().next_tid, event->data().cpu,
                                           event->timestamp());
  }

  // Process the context switch out for thread state.
  if (event->data().prev_tid != 0 && TidMatchesPidFilter(event->data().prev_tid)) {
    ThreadStateSlice::ThreadState new_state = GetThreadStateFromBits(event->data().prev_state);
    std::optional<ThreadStateSlice> out_slice =
        state_manager_.OnSchedSwitchOut(event->timestamp(), event->data().prev_tid, new_state);
  ...
  if (event->data().next_tid != 0 && TidMatchesPidFilter(event->data().next_tid)) {
    std::optional<ThreadStateSlice> in_slice =
        state_manager_.OnSchedSwitchIn(event->timestamp(), event->data().next_tid);
...
void SwitchesStatesNamesVisitor::Visit(SchedWakeupPerfEvent* event) {
  if (!TidMatchesPidFilter(event->data().woken_tid)) {
    return;
  }

  std::optional<ThreadStateSlice> state_slice =
      state_manager_.OnSchedWakeup(event->timestamp(), event->data().woken_tid);
...
void SwitchesStatesNamesVisitor::Visit(TaskRenamePerfEvent* event) {
  std::optional<pid_t> renamed_pid = GetPidOfTid(event->data().renamed_tid);
  ThreadName thread_name;
  thread_name.set_pid(renamed_pid.value_or(-1));
  thread_name.set_tid(event->data().renamed_tid);
  thread_name.set_name(event->data().newcomm);
  thread_name.set_timestamp_ns(event->timestamp());
  listener_->OnThreadName(std::move(thread_name));
}
