...
void TracerImpl::Startup() {
  ...
  auto maps_or_error = orbit_object_utils::ReadMaps(target_pid_);
  if (maps_or_error.has_value()) {
    std::vector<ModuleInfo> modules =
        orbit_object_utils::ParseMapsIntoModules(maps_or_error.value());
    *modules_snapshot.mutable_modules() = {modules.begin(), modules.end()};
    ...
    if (!uprobes_errors) {
      std::map<uint64_t, std::string> function_ids_to_error_messages =
          FindFunctionsThatUprobesCannotInstrumentWithMessages(maps_or_error.value(),
