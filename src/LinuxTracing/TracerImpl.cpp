...
uint64_t TracerImpl::ProcessForkEventAndReturnTimestamp(const perf_event_header& header,
  ...
  PerfEvent event{.timestamp = timestamp,
...
uint64_t TracerImpl::ProcessMmapEventAndReturnTimestamp(const perf_event_header& header,
                                                        PerfEventRingBuffer* ring_buffer) {
  PerfEvent event = ConsumeMmapPerfEvent(ring_buffer, header);
  const uint64_t timestamp_ns = event.timestamp;

  if (std::get<MmapPerfEvent>(event.data).pid != target_pid_) {
    return timestamp_ns;
  }
...
uint64_t TracerImpl::ProcessSampleEventAndReturnTimestamp(const perf_event_header& header,
  ...
  if (is_uprobe) {
    ...
    PerfEvent event{
        ...
        .ordered_in_file_descriptor = fd,
        .data =
            UretprobesWithReturnValuePerfEvent{
                .pid = static_cast<pid_t>(ring_buffer_record.sample_id.pid),
                .tid = static_cast<pid_t>(ring_buffer_record.sample_id.tid),
    ...
    PerfEvent event = ConsumeStackSamplePerfEvent(ring_buffer, header);
    DeferEvent(std::move(event));
    ++stats_.sample_count;
    ...
    memcpy(std::get<TaskNewtaskPerfEvent>(event.data).comm, ring_buffer_record.data.comm, 16);
    DeferEvent(std::move(event));
    ...
    memcpy(std::get<TaskRenamePerfEvent>(event.data).newcomm, ring_buffer_record.data.newcomm, 16);
    DeferEvent(std::move(event));
    ...
    PerfEvent event = ConsumeAmdgpuCsIoctlPerfEvent(ring_buffer, header);
    DeferEvent(std::move(event));
    ++stats_.gpu_events_count;

  } else if (is_amdgpu_sched_run_job_event) {
    PerfEvent event = ConsumeAmdgpuSchedRunJobPerfEvent(ring_buffer, header);
    DeferEvent(std::move(event));
    ++stats_.gpu_events_count;

  } else if (is_dma_fence_signaled_event) {
    PerfEvent event = ConsumeDmaFenceSignaledPerfEvent(ring_buffer, header);
    DeferEvent(std::move(event));
    ++stats_.gpu_events_count;
    ...
    PerfEvent event = ConsumeGenericTracepointPerfEvent(ring_buffer, header);
    const GenericTracepointPerfEvent& event_data = std::get<GenericTracepointPerfEvent>(event.data);

    orbit_grpc_protos::FullTracepointEvent tracepoint_event;
    tracepoint_event.set_pid(event_data.pid);
    tracepoint_event.set_tid(event_data.tid);
    tracepoint_event.set_timestamp_ns(event.timestamp);
    tracepoint_event.set_cpu(event_data.cpu);
...
uint64_t TracerImpl::ProcessLostEventAndReturnTimestamp(const perf_event_header& header,
  ...
  std::get<LostPerfEvent>(event.data).previous_timestamp = fd_previous_timestamp_ns;
  DeferEvent(std::move(event));
