...
void ExpectIntervalEQ(const BinomialConfidenceInterval& interval, float expected_lower,
...
TEST(WilsonBinomialConfidenceIntervalEstimatorTest, LargeSampleSmallProbabilityTest) {
  auto interval = kEstimator.Estimate(0.025f, 2800);
...
TEST(WilsonBinomialConfidenceIntervalEstimatorTest, ShortSampleSmallProbabilityTest) {
  auto interval = kEstimator.Estimate(0.1f, 10);
