...
const orbit_client_protos::TimerInfo* SnapToClosestStart(const TimeGraph* time_graph,
...
bool LiveFunctionsController::OnAllNextButton() {
  ...
  for (auto it : iterator_id_to_function_id_) {
    ...
    const orbit_client_protos::TimerInfo* timer_info =
        app_->GetTimeGraph()->FindNextFunctionCall(function_id, current_timer_info->end());
...
bool LiveFunctionsController::OnAllPreviousButton() {
  ...
  for (auto it : iterator_id_to_function_id_) {
    uint64_t function_id = it.second;
    const orbit_client_protos::TimerInfo* current_timer_infos =
        current_timer_infos_.find(it.first)->second;
    const orbit_client_protos::TimerInfo* timer_info =
        app_->GetTimeGraph()->FindPreviousFunctionCall(function_id, current_timer_infos->end());
    if (timer_info == nullptr) {
      return false;
    }
    if (timer_info->start() < min_timestamp) {
...
void LiveFunctionsController::OnNextButton(uint64_t id) {
  const orbit_client_protos::TimerInfo* timer_info = app_->GetTimeGraph()->FindNextFunctionCall(
...
void LiveFunctionsController::OnPreviousButton(uint64_t id) {
  const orbit_client_protos::TimerInfo* timer_info = app_->GetTimeGraph()->FindPreviousFunctionCall(
      iterator_id_to_function_id_[id], current_timer_infos_[id]->end());
...
void LiveFunctionsController::AddIterator(uint64_t function_id, const FunctionInfo* function) {
  ...
  if (!timer_info || timer_info->function_id() != function_id) {
    timer_info = SnapToClosestStart(app_->GetTimeGraph(), function_id);
  }
