...
std::string ThreadTrack::GetBoxTooltip(const Batcher& batcher, PickingId id) const {
  ...
  if (!timer_info || timer_info->type() == TimerInfo::kCoreActivity) {
    return "";
  }
...
void ThreadTrack::OnTimer(const TimerInfo& timer_info) {
  ...
  // Pass ownership to timer_chain. TODO: Pass timer_info as a value instead of reference order to
  // be able to move it.
  auto& timer_info_chain_ref = timer_chain->emplace_back(timer_info);
  ++num_timers_;
