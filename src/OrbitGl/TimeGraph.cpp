...
void TimeGraph::VerticalZoom(float zoom_value, float mouse_normalized_y_position) {
  ...
  const float offset_from_top_in_world =
      viewport_->GetVisibleWorldHeight() * mouse_normalized_y_position;
  const float total_mouse_y_position =
      (offset_from_top_in_world + vertical_scrolling_offset_) / old_scale;
  const float new_scrolling_offset =
      total_mouse_y_position * layout_.GetScale() - offset_from_top_in_world;
...
void TimeGraph::VerticallyMoveIntoView(Track& track) {
  ...
  SetVerticalScrollingOffset(
      std::clamp(vertical_scrolling_offset_, min_world_top_left_y, max_world_top_left_y));
}
...
float TimeGraph::GetWorldFromTick(uint64_t time) const {
  if (time_window_us_ > 0) {
    ...
    auto pos = float(normalized_start * GetWidth());
    return pos;
  }
...
void TimeGraph::SetVerticalScrollingOffset(float value) {
  float clamped_value =
      std::max(std::min(value, GetHeight() - viewport_->GetVisibleWorldHeight()), 0.f);
