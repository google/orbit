...
void EnqueueEncodedEvent(orbit_api::ApiEvent& event) {
  // One api event per capture event sent to OrbitService
  // static orbit_api::LockFreeApiEventProducer producer;

  // Many api events per capture event sent to OrbitService
  static orbit_api::LockFreeApiEventBulkProducer producer;

  event.pid = orbit_base::GetCurrentProcessId();
  event.tid = orbit_base::GetCurrentThreadId();
  event.timestamp_ns = MonotonicTimestampNs();
...
void orbit_api_start(const char* name, orbit_api_color color) {
  orbit_api::ApiEvent e(orbit_api::EventType::kScopeStart, name, kDataZero, color);
  // orbit_api::Start(e.args[0], e.args[1], e.args[2], e.args[3], e.args[4], e.args[5]);
  EnqueueEncodedEvent(e);
}

void orbit_api_stop() {
  orbit_api::ApiEvent e(orbit_api::EventType::kScopeStop);
...
void orbit_api_start_async(const char* name, uint64_t id, orbit_api_color color) {
  orbit_api::ApiEvent e(orbit_api::EventType::kScopeStartAsync, name, id, color);
  // orbit_api::StartAsync(e.args[0], e.args[1], e.args[2], e.args[3], e.args[4], e.args[5]);
  EnqueueEncodedEvent(e);
}

void orbit_api_stop_async(uint64_t id) {
  orbit_api::ApiEvent e(orbit_api::EventType::kScopeStopAsync, kNameNullPtr, id);
...
void orbit_api_async_string(const char* str, uint64_t id, orbit_api_color color) {
  ...
  while (str < end) {
    orbit_api::ApiEvent e(orbit_api::EventType::kString, kNameNullPtr, id, color);
    std::strncpy(e.event.event.name, str, chunk_size);
    e.event.event.name[chunk_size] = 0;
    EnqueueEncodedEvent(e);
...
static inline void TrackValue(orbit_api::EventType type, const char* name, uint64_t value,
  ...
  orbit_api::ApiEvent e(type, name, value, color);
  // orbit_api::TrackValue(e.args[0], e.args[1], e.args[2], e.args[3], e.args[4], e.args[5]);
  EnqueueEncodedEvent(e);
}
