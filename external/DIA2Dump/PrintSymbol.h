...
void GetTypeInformation(class Type* a_Type,
                        std::shared_ptr<OrbitDiaSymbol> pSymbol,
                        DWORD a_TagType, DWORD dwIndent);

template <class T>
void PrintGeneric(T t) {
  IDiaPropertyStorage* pPropertyStorage;

  if (t->QueryInterface(__uuidof(IDiaPropertyStorage),
...
struct VizLogger : public OrbitLogger {
  void Log(const std::wstring& a_Log) override { ORBIT_PRINTF(ws2s(a_Log)); }
};
...
struct StringLogger : public OrbitLogger {
  void Log(const std::wstring& a_Log) override { m_String += a_Log; }
