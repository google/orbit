...
bool FunctionsDataView::ShowSelectedFunctionIcon(const FunctionInfo& function) {
  return GOrbitApp->IsFunctionSelected(function);
}

bool FunctionsDataView::ShowFrameTrackIcon(const FunctionInfo& function) {
  if (GOrbitApp->IsFrameTrackEnabled(function)) {
    return true;
  } else if (GOrbitApp->HasCaptureData()) {
...
std::string FunctionsDataView::BuildSelectedColumnsString(const FunctionInfo& function) {
  ...
  if (ShowSelectedFunctionIcon(function)) {
    absl::StrAppend(&result, kSelectedFunctionString);
    if (ShowFrameTrackIcon(function)) {
      absl::StrAppend(&result, " ", kFrameTrackString);
    }
  } else if (ShowFrameTrackIcon(function)) {
    absl::StrAppend(&result, kFrameTrackString);
  }
  return result;
}
