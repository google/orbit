    ...
    for (TimerChainIterator it = chain->begin(); it != chain->end(); ++it) {
      ...
      for (size_t k = 0; k < block.size(); ++k) {
        ...
        bool need_highlight = GOrbitApp->selected_text_box() && 
			...
			timer_info.function_address() == GOrbitApp->selected_text_box()->GetTimerInfo().function_address();
        need_highlight =
            need_highlight || (GOrbitApp->highlighted_function() > 0 && GOrbitApp->highlighted_function() == timer_info.function_address());
