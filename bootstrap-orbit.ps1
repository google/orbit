...
"@
  ...
  $result = Start-Process -FilePath $pip3.Path -ArgumentList "install","conan==1.29.2" -Wait -NoNewWindow -ErrorAction Stop -PassThru
...
"@
  ...
  if ($conan_version_major -eq $conan_version_major_required -and $conan_version_minor -lt $conan_version_minor_min) {
    ...
    Try {
      ...
      Start-Process -FilePath $pip3.Path -ArgumentList "install","--upgrade","conan==1.29.2" -Wait -NoNewWindow -PassThru
    } Catch {
