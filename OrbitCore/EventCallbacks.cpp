...
void EventTracing::Init() {
  if (GEventCallbacks.size() == 0) {
    ...
    GEventCallbacks[EventGuid::Hash(EventTraceConfigGuid)] =
        EventTracing::CallbackEventTraceConfig;
    GEventCallbacks[EventGuid::Hash(FileIoGuid)] = EventTracing::CallbackFileIo;
    GEventCallbacks[EventGuid::Hash(ImageLoadGuid)] =
        EventTracing::CallbackImageLoad;
    GEventCallbacks[EventGuid::Hash(PageFaultGuid)] =
        EventTracing::CallbackPageFault;
    GEventCallbacks[EventGuid::Hash(PerfInfoGuid)] =
        EventTracing::CallbackPerfInfo;
    GEventCallbacks[EventGuid::Hash(ProcessGuid)] =
        EventTracing::CallbackProcess;
    GEventCallbacks[EventGuid::Hash(RegistryGuid)] =
        EventTracing::CallbackRegistry;
    GEventCallbacks[EventGuid::Hash(SplitIoGuid)] =
        EventTracing::CallbackSplitIo;
    ...
    GEventCallbacks[EventGuid::Hash(StackWalkGuid)] =
        EventTracing::CallbackStackWalk;
  }
}
...
void EventTracing::Callback(PEVENT_RECORD a_EventRecord) {
  if (Capture::GTargetProcess && Capture::IsCapturing()) {
    ...
    if (EventCallback& callback = GEventCallbacks[idHash]) {
      callback(a_EventRecord,
               a_EventRecord->EventHeader.EventDescriptor.Opcode);
    }
  }
}
...
//-----------------------------------------------------------------------------
void EventTracing::CallbackEventTraceConfig(PEVENT_RECORD a_EventRecord,
                                            UCHAR a_Opcode) {}
...
//-----------------------------------------------------------------------------
void EventTracing::CallbackImageLoad(PEVENT_RECORD a_EventRecord,
                                     UCHAR a_Opcode) {}

//-----------------------------------------------------------------------------
void EventTracing::CallbackPageFault(PEVENT_RECORD a_EventRecord,
                                     UCHAR a_Opcode) {}

//-----------------------------------------------------------------------------
void EventTracing::CallbackPerfInfo(PEVENT_RECORD a_EventRecord,
                                    UCHAR a_Opcode) {
...
//-----------------------------------------------------------------------------
void EventTracing::CallbackProcess(PEVENT_RECORD a_EventRecord,
                                   UCHAR a_Opcode) {}

//-----------------------------------------------------------------------------
void EventTracing::CallbackRegistry(PEVENT_RECORD a_EventRecord,
                                    UCHAR a_Opcode) {}

//-----------------------------------------------------------------------------
void EventTracing::CallbackSplitIo(PEVENT_RECORD a_EventRecord,
                                   UCHAR a_Opcode) {}
...
void EventTracing::CallbackThread(PEVENT_RECORD a_EventRecord, UCHAR a_Opcode) {
  switch (a_Opcode) {
    ...
    case Thread_TypeGroup1::OPCODE_DC_END: {
      ...
      {
        // Thread started
        if (a_Opcode == Thread_TypeGroup1::OPCODE_START ||
            a_Opcode == Thread_TypeGroup1::OPCODE_DC_START) {
...
//-----------------------------------------------------------------------------
void EventTracing::CallbackStackWalk(PEVENT_RECORD a_EventRecord,
                                     UCHAR a_Opcode) {
  if (a_Opcode == StackWalk_Event::OPCODE_STACK) {
    ...
    if (event->StackProcess == Capture::GTargetProcess->GetID()) {
      int stackDepth = (a_EventRecord->UserDataLength -
                        (sizeof(StackWalk_Event) - sizeof(event->Stack1))) /
                       sizeof(ULONG64);
      ...
      size_t numBytes =
          std::min(stackDepth, ORBIT_STACK_SIZE) * sizeof(ULONG64);
      ...
      GEventTracer.GetEventBuffer().AddCallstackEvent(
          a_EventRecord->EventHeader.TimeStamp.QuadPart, CS.Hash(),
          CS.m_ThreadId);
    }
  }
}
...
inline void ProcessProfileEvent(PEVENT_RECORD a_EventRecord) {
  ...
  LONGLONG CycleTime = Header.TimeStamp.QuadPart;
  PerfInfo_SampledProfile* sampleEvent =
      (PerfInfo_SampledProfile*)a_EventRecord->UserData;
