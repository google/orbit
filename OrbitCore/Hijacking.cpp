...
struct ThreadLocalData {
  ...
  void SendThreadInfo() {
    if (!m_ThreadName.empty()) {
      Message msg(Msg_ThreadInfo,
                  uint32_t((m_ThreadName.size() + 1) * sizeof(m_ThreadName[0])),
                  (char*)m_ThreadName.data());
...
void Prolog(void* a_OriginalFunctionAddress, Context* a_Context,
            unsigned a_ContextSize);
void PrologZoneStart(void* a_OriginalFunctionAddress, Context* a_Context,
                     unsigned a_ContextSize);
void PrologZoneStop(void* a_OriginalFunctionAddress, Context* a_Context,
                    unsigned a_ContextSize);
void PrologOutputDbg(void* a_OriginalFunctionAddress, Context* a_Context,
                     unsigned a_ContextSize);
void PrologSendData(void* a_OriginalFunctionAddress, Context* a_Context,
                    unsigned a_ContextSize);
void PrologUnrealActor(void* a_OriginalFunctionAddress, Context* a_Context,
                       unsigned a_ContextSize);
void PrologFree(void* a_OriginalFunctionAddress, Context* a_Context,
                unsigned a_ContextSize);
...
__forceinline CallstackID SendCallstack(void* a_OriginalFunctionAddress,
...
__forceinline void PushZoneContext(const Context* a_Context,
                                   void* a_OriginalFunction);
__forceinline void PopContext();
__forceinline void SendContext(const Context* a_Context,
                               EpilogContext* a_EpilogContext);
...
// On Win64, epilog context is at 40 bytes: 8 bytes (return address) + 32 bytes
// (shadow space) On Win32, epilog context is at 4 bytes (return address)
...
//-----------------------------------------------------------------------------
__forceinline CallstackID Hijacking::SendCallstack(
    void* a_OriginalFunctionAddress, void** a_ReturnAddressLocation) {
  /*bool needsCallstack = m_SendCallstacks.find( reinterpret_cast<ULONG64>(
  a_OriginalFunctionAddress ) ) != m_SendCallstacks.end(); if( needsCallstack
  )*/
  {
...
//-----------------------------------------------------------------------------
void Hijacking::Prolog(void* a_OriginalFunctionAddress, Context* a_Context,
                       unsigned a_ContextSize) {
  SSE_SCOPE;
  CheckTls();
  ...
  TlsData->m_Timers.back().m_FunctionAddress =
      reinterpret_cast<ULONG64>(a_OriginalFunctionAddress);
...
//-----------------------------------------------------------------------------
void Hijacking::PrologZoneStart(void* a_OriginalFunctionAddress,
                                Context* a_Context, unsigned a_ContextSize) {
  SSE_SCOPE;
  CheckTls();
...
//-----------------------------------------------------------------------------
void Hijacking::PrologZoneStop(void* a_OriginalFunctionAddress,
                               Context* a_Context, unsigned a_ContextSize) {
  SSE_SCOPE;
  CheckTls();
  PushReturnAddress(&a_Context->m_RET.m_Ptr);
...
//-----------------------------------------------------------------------------
void Hijacking::PrologOutputDbg(void* a_OriginalFunctionAddress,
                                Context* a_Context, unsigned a_ContextSize) {
  SSE_SCOPE;
  CheckTls();
  OrbitLogEntry entry;
  ...
  entry.m_CallstackHash =
      SendCallstack(a_OriginalFunctionAddress, &a_Context->m_RET.m_Ptr);
  GTcpClient->Send(entry);
}

//-----------------------------------------------------------------------------
void Hijacking::PrologSendData(void* a_OriginalFunctionAddress,
                               Context* a_Context, unsigned a_ContextSize) {
  SSE_SCOPE;
  CheckTls();
  ...
  // __declspec( noinline ) inline void OrbitSendData( void*, int)  {
  // ORBIT_NOOP; }
...
//-----------------------------------------------------------------------------
void Hijacking::PrologUnrealActor(void* a_OriginalFunctionAddress,
                                  Context* a_Context, unsigned a_ContextSize) {
  SSE_SCOPE;
  CheckTls();
...
__forceinline void Hijacking::SendUObjectName(void* a_UObject) {
  if (a_UObject) {
    ...
    if (TlsData->m_SentActorNames.find(actorName) ==
        TlsData->m_SentActorNames.end()) {
      ...
      if (IsWide) {
        ...
        Message msg(Msg_OrbitUnrealObject, (int)(numChars * sizeof(wchar_t)),
                    (char*)actorNameW);
...
//-----------------------------------------------------------------------------
void Hijacking::PrologFree(void* a_OriginalFunctionAddress, Context* a_Context,
                           unsigned a_ContextSize) {
  SSE_SCOPE;
  CheckTls();
...
__forceinline void Hijacking::SetOriginalReturnAddresses() {
  ...
  // Make sure we don't interpret the last written address as an overwritten
  // address.
...
void* Hijacking::Epilog() {
  ...
  EpilogContext* epilogContext =
      (EpilogContext*)((char*)stackAddress + s_StackOffset);

  SendContext(TlsData->m_Contexts.back(), epilogContext);
  PopContext();

  void* ReturnAddress =
      TlsData->m_ReturnAdresses.back().m_OriginalReturnAddress;
...
void* Hijacking::EpilogZoneStop() {
  ...
  if (TlsData->m_ZoneStack > 0) {
    ...
    if (TlsData->m_SentLiterals.find(zoneName) ==
        TlsData->m_SentLiterals.end()) {
      size_t numChars =
          std::min(strlen(zoneName), size_t(OrbitZoneName::NUM_CHAR - 1));
      OrbitZoneName zone;
#ifdef _WIN64
...
//-----------------------------------------------------------------------------
__forceinline void Hijacking::PushReturnAddress(
    void** a_AddressOfReturnAddress) {
...
//-----------------------------------------------------------------------------
__forceinline void Hijacking::PushContext(const Context* a_Context,
                                          void* a_OriginalFunction) {
  TlsData->m_Contexts.push_back(a_Context);

  // Note: disabled argument tracking in favor of better perf.  It required work
  // anyway, will re-enable. FunctionArgInfo* argInfo = GetArgInfo(
  // a_OriginalFunction ); context->m_StackSize = argInfo ?
  // argInfo->m_NumStackBytes : 0; memcpy( context, a_Context,
  // Context::GetFixedDataSize() + context->m_StackSize ); context->m_RET =
  // (AddressType)a_OriginalFunction;
}

//-----------------------------------------------------------------------------
__forceinline void Hijacking::PushZoneContext(const Context* a_Context,
                                              void* a_OriginalFunction) {
  TlsData->m_Contexts.push_back(a_Context);
}

//-----------------------------------------------------------------------------
__forceinline void Hijacking::PopContext() { TlsData->m_Contexts.pop_back(); }

//-----------------------------------------------------------------------------
inline void Hijacking::SendContext(const Context* a_Context,
                                   EpilogContext* a_EpilogContext) {
#if 0
...
//-----------------------------------------------------------------------------
bool Hijacking::Deinitialize() { return MH_Uninitialize() == MH_OK; }
...
//-----------------------------------------------------------------------------
bool Hijacking::CreateHook(void* a_FunctionAddress, void* a_PrologCallback,
                           void* a_EpilogCallback) {
  Initialize();
  MH_STATUS MinHookError = MH_Orbit_CreateHookPrologEpilog(
      a_FunctionAddress, a_PrologCallback, a_EpilogCallback, &GetReturnAddress);
...
//-----------------------------------------------------------------------------
bool Hijacking::DisableAllHooks() { return MH_DisableAllHooks() == MH_OK; }
...
//-----------------------------------------------------------------------------
void Hijacking::SetFunctionArguments(ULONG64 a_FunctionAddress,
...
void Hijacking::SetUnrealInfo(OrbitUnrealInfo& a_UnrealInfo) {
  ...
  GetDisplayNameEntry = (GetDisplayNameEntryFunction)(
                            void*)a_UnrealInfo.m_GetDisplayNameEntryAddress;
}
