...
//-----------------------------------------------------------------------------
template<typename T> struct OrbitDiaSmartPtr
{
	...
	~OrbitDiaSmartPtr(){ if( m_Symbol ) OrbitDiaReleasePtr( (IUnknown*)m_Symbol ); OrbitSmartPtrDestroyed(); }
	void Release() { OrbitDiaReleasePtr( m_Symbol ); m_Symbol = nullptr; OrbitSmartPtrDestroyed(); }
