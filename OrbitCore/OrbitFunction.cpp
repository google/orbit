...
void Function::FindFile() {
  ...
  if (lineInfo.m_File != L"") m_File = ws2s(lineInfo.m_File);
...
const TCHAR* Function::GetCallingConventionString(int a_CallConv) {
  const TCHAR* CallingConv[] = {
      ...
      L"INLINE",  // CV_CALL_INLINE      = 0x17, // Marker for routines always
...
void Function::Print() {
  ...
  std::shared_ptr<OrbitDiaSymbol> diaSymbol =
      m_Pdb->GetDiaSymbolFromId(this->m_Id);
