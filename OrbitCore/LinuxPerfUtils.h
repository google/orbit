...
namespace LinuxPerfUtils
{
    ...
    // Sample all pointers: they might all be necessary later for DWARF-based stack unwinding.
    static constexpr uint64_t SAMPLE_REGS_USER_ALL =
        ...
        (1lu << PERF_REG_X86_SP) |
        (1lu << PERF_REG_X86_IP) |
        (1lu << PERF_REG_X86_FLAGS) |
        (1lu << PERF_REG_X86_CS) |
        (1lu << PERF_REG_X86_SS) |
        (1lu << PERF_REG_X86_R8) |
        (1lu << PERF_REG_X86_R9) |
        (1lu << PERF_REG_X86_R10) |
        (1lu << PERF_REG_X86_R11) |
        (1lu << PERF_REG_X86_R12) |
        (1lu << PERF_REG_X86_R13) |
        (1lu << PERF_REG_X86_R14) |
        (1lu << PERF_REG_X86_R15);
    ...
    inline int32_t perf_event_open(struct perf_event_attr* a_HWEvent,
            pid_t a_PID, int32_t a_CPU, int32_t a_GroupFD, uint32_t a_Flags) {
        return syscall(__NR_perf_event_open,
                       a_HWEvent, a_PID, a_CPU, a_GroupFD, a_Flags);
    }
    ...
    int32_t tracepoint_event_open(uint64_t a_TracepointID,
