...
function format_annotations() {
  ...
  echo -n "{"
  echo -n "\"name\":\"missing-license-headers\","
  echo -n "\"head_sha\":\"$COMMIT_SHA\","
  echo -n "\"status\":\"completed\","
  echo -n "\"conclusion\":\"failure\","
  echo -n "\"output\":{"
  echo -n "\"title\":\"Missing license headers report\","
  echo -n "\"summary\":\"You are missing the license headers in the following files.\","
  echo -n "\"annotations\":["
  FIRST_LINE=1
...
format_annotations "${GITHUB_WORKSPACE}/missing_license_headers.txt" | curl \
  ...
  https://api.github.com/repos/florian-kuebler/orbitprofiler/check-runs \
  -d @-