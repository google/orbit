...
void OrbitProlog::Init() {
  if (m_Data.m_Code == nullptr) {
    ...
    std::vector<size_t> offsets =
        FindOffsets(m_Data.m_Code, Prolog_NumOffsets, dummyAddress);

    if (offsets.size() != Prolog_NumOffsets) {
      OutputDebugStringA(
          "OrbitAsm: Did not find expected number of offsets in the Prolog\n");
      return;
    }
...
void OrbitEpilog::Init() {
  if (!m_Data.m_Code) {
    ...
    std::vector<size_t> offsets =
        FindOffsets(m_Data.m_Code, Epilog_NumOffsets, dummyAddress);

    if (offsets.size() != Epilog_NumOffsets) {
      OutputDebugStringA(
          "OrbitAsm: Did not find expected number of offsets in the Epilog\n");
      return;
    }
...
const Prolog* GetOrbitProlog() {
  if (!GProlog.m_Data.m_Code) GProlog.Init();
  return &GProlog.m_Data;
}

//-----------------------------------------------------------------------------
const Epilog* GetOrbitEpilog() {
  if (!GEpilog.m_Data.m_Code) GEpilog.Init();
