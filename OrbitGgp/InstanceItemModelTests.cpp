...
TEST(InstanceItemModelTests, index) {
  InstanceItemModel model{};
...
TEST(InstanceItemModelTests, headerData) {
  InstanceItemModel model{};
...
TEST(InstanceItemModelTests, data) {
  ...
  ASSERT_EQ(cell_0_0_user_data.userType(), qMetaTypeId<Instance>());

  const auto cell_0_0_ggp_instance = cell_0_0_user_data.value<Instance>();
  ...
  ASSERT_EQ(cell_1_0_user_data.userType(), qMetaTypeId<Instance>());

  const auto cell_1_0_ggp_instance = cell_1_0_user_data.value<Instance>();
...
TEST(InstanceItemModelTests, SetInstances) {
  ...
  QVector<Instance> test_instances{Instance{}};
  InstanceItemModel model{test_instances};
  ...
  // add 1 instance
  test_instances.push_back(Instance{});
  reset_counters();
  model.SetInstances(test_instances);
